...
/* Every caller-supplied class name becomes a path under the dev sandbox, so it
 * is validated BEFORE it is joined: the character class admits no `/`, `\` or
 * `.`, which is what keeps a name like `../../src/01/z2ui5_cl_x` from reaching
 * a file outside src/zz_dev. Shared by deploy and remove — the write path and
 * the delete path must not disagree about what a legal name is. */
...
// the Z2UI5_CL_SMPC_ port prefix, which user apps must not be forced into)
