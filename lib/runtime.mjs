...
// the deploy sandbox and build pipeline live in the ai-demokit checkout
function demokit() {
  const d = resolveAiDemokit();
  if (!d) throw new Error('ai-demokit checkout not found — set AI_DEMOKIT_HOME or clone it as a sibling');
  return d;
}
...
export async function lintApp(className) {
  const cls = String(className || '').toLowerCase();
  const { stdout, stderr, timedOut } = await spawnWithTimeout(
    'npx',
    ['abaplint', devLintConfig(), '--format', 'json'],
    { cwd: demokit(), timeoutMs: timeoutOf('A2UI5_MCP_LINT_TIMEOUT_MS') },
  );
  if (timedOut) {
    return {
      ok: false,
...
// in/out-of-scope verdict for UI5 control entities, via ai-demokit's
...
export async function runScopeOf(entities) {
  const { code, stdout, stderr, timedOut } = await spawnWithTimeout(
    'node',
    [path.join(demokit(), 'scripts', 'scope-of.mjs'), ...entities],
    { cwd: demokit(), timeoutMs: timeoutOf('A2UI5_MCP_SCOPE_TIMEOUT_MS') },
  );
...
export function buildBackend({ onLine, mode = 'auto' } = {}) {
  ...
  if (incremental && !a2) {
    return Promise.resolve({ ok: false, code: 1, tail: 'abap2UI5 checkout not found — clone https://github.com/abap2UI5/abap2UI5 as a sibling of ai-mcp (or run `npm run node:setup` in ai-demokit), or point A2UI5_HOME at an existing checkout; then run mode:full once' });
  }
    ...
    try {
      ...
      if (incremental) {
        ...
        if (!fs.existsSync(path.join(lib, 'src'))) {
          ...
          execSync(`node ${path.join(demokit(), "web/ci/patch_open_abap_xml.mjs")} ${lib}`, { cwd: a2, timeout: buildTimeout });
        }
        ...
        cmd = 'node';
        cmdArgs = [path.join(demokit(), "scripts", "e2e-build.mjs")];
        cwd = demokit();
      }
      ...
      if (timedOut) {
        return {
          ...
          mode,
          timedOut: true,
          tail: [...tail, timedOutError(`${mode} build`, 'A2UI5_MCP_BUILD_TIMEOUT_MS')].join('\n'),
        };
      }
      return { ok: code === 0, code, mode, tail: tail.join('\n') };
    } finally {
      building = null;
...
// benign-noise rules: the canonical list is ai-demokit's scripts/lib-smoke.mjs
