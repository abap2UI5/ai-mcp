 ...
 *   samples-controls  431 ports - the UI5 demo kit rebuilt control by control.
 *                     The answer to "how do I express sap.m.Wizard", which the
 ...
 *   { repo, area, section, title, sub, summary, label, keywords, cls, path }
 */
import fs from 'fs';
 ...
 */
const ROW = /^\|\s*(?:\*\*(?<title>[^*]+)\*\*\s*(?:—|--)\s*)?(?<sub>[^|<]*?)\s*(?<blocks>(?:<br>(?:<sub>[^<]*<\/sub>|[^<]*))*)\s*\|\s*\[`(?<cls>[A-Z0-9_]+)`\]\((?<path>[^)]+)\)\s*\|/;

/** The blocks under a row title, in order, as `{ small, text }`. */
function blocksOf(blocks) {
  return [...(blocks || '').matchAll(/<br>(?:<sub>([^<]*)<\/sub>|([^<]*))/g)]
    .map((m) => (m[1] !== undefined
