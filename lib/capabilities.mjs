...
function capabilitiesFile() {
  ...
  if (!demokit) throw new Error('ai-demokit checkout not found — set AI_DEMOKIT_HOME or clone it as a sibling (the capability map lives there)');
...
export function parseCapabilities() {
  const text = fs.readFileSync(capabilitiesFile(), 'utf8');
  const entries = [];
  let section = '';
